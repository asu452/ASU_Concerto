		...
		if($this->ctype == 'text'){
			...
			if($this->content_i['error'] == 0 && is_uploaded_file($this->content_i['tmp_name'])){
				...
				if($pre_type == "image/jpeg" || $pre_type == "image/pjpeg" || $pre_type == "image/jpg"){
					...
					$this->ppt_cleaner();
                                //I added the next elseif for .pptx files - Lucas Pate
                                } elseif ($pre_type == "application/vnd.openxmlformats-officedocument.presentationml.presentation"){
                                        $this->ppt_cleaner();
				} elseif ($pre_type == "application/pdf"){
					$this->pdf_cleaner(); 
//*****************************************************************************
//testing html with the next elseif
//*****************************************************************************  
                                } elseif ($pre_type == "text/html"){
                               $this->content_o = $this->content_i;
                                $this->mime_type = 'text/html';
                                 $this->type_id = 2; //SELF: THIS IS BAD AND DUMB AND STUPID
                                $content = new Content();
